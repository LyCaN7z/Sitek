...
def user_guard(func):
    """
    All user commands တွင် ဤ decorator ကို ချိတ်မည်:
    ① bot_enabled=False → user သုံးလို့မရ (admin သုံးလို့ရ)
    ② banned=True       → user blocked
    ③ daily limit ပြည့်  → scan/tool commands အတွက်ပါ count
    """
    @functools.wraps(func)
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Admin ဆိုရင် ဘာ check မှ မလုပ်
        if uid in ADMIN_IDS:
            return await func(update, context)
        ...
        # Bot off check
        if not db["settings"].get("bot_enabled", True):
            await update.effective_message.reply_text(
                "🔴 *Bot ယာယီပိတ်ထားပါတယ်*\n\n"
                "Admin မှ ဖွင့်မည့်အချိန်ကို ခဏစောင့်ပါ။",
                parse_mode='Markdown'
            )
            return
        ...
        if u.get("banned"):
            await update.effective_message.reply_text(
                "🚫 *သင့် account ကို block ထားပါတယ်*\n"
                "Admin ထံ ဆက်သွယ်ပါ။",
                parse_mode='Markdown'
            )
            return
...
async def _run_download(
    ...
    async with db_lock:
        ...
        if not resume_mode and not can_download(db, u):
            ...
            _save_db_sync(db)
            await update.effective_message.reply_text(
                f"⛔ *Daily Limit ပြည့်ပါပြီ*\n\n"
                f"📊 သင်သုံးခဲ့သည်: `{u.get('count_today',0)}/{lim}` downloads\n"
                f"🕛 မနက်ဖြန် midnight တွင် အလိုအလျောက် reset ဖြစ်မည်\n\n"
                f"💡 Admin မှ `/resetlimit {uid}` နှိပ်ပါက ယခုပဲ ပြန်သုံးနိုင်မည်",
                parse_mode='Markdown'
            ); return
        _save_db_sync(db)
...
# ══════════════════════════════════════════════════
# 🔑  KEY EXTRACTOR ENGINE  — Deep Scan (HTML + JS bundles)
# ══════════════════════════════════════════════════

import base64 as _b64

# ─── Core fetch: HTML + all linked JS files ───────────────────

def _scrape_full(url: str, max_js: int = 12) -> dict:
    """
    Fetch the target URL and all linked JS/script files.
    Returns: {
        "html":        str,
        "headers":     dict,
        "status":      int,
        "cookies":     dict,
        "js_sources":  [(js_url, js_text), ...],
        "all_text":    str   # HTML + all JS concatenated
    }
    """
    result = {"html": "", "headers": {}, "status": 0,
    ...
    try:
        ...
        for tag in soup.find_all("script", src=True):
            ...
            if js_url not in seen and js_url.startswith("http"):
                seen.add(js_url)
                js_urls.append(js_url)

        # ── Also regex-scan for dynamically loaded JS ────────
        for m in re.finditer(
            r'["\`]((?:https?:)?//[^"\'`\s<>]+\.js(?:\?[^"\'`\s<>]*)?)["\`]',
            resp.text
        ):
        ...
        def _fetch_one_js(js_url):
            try:
                r = sess.get(js_url, timeout=10, verify=False,
                             allow_redirects=True, proxies=proxy)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=6) as ex:
            futures = [ex.submit(_fetch_one_js, u) for u in js_urls[:max_js]]
            for fut in concurrent.futures.as_completed(futures, timeout=20):
                res = fut.result()
                if res:
                    result["js_sources"].append(res)

        all_parts = [result["html"]] + [js for _, js in result["js_sources"]]
        result["all_text"] = "\n".join(all_parts)
...
def _scan_text(corpus: str, patterns: dict) -> dict:
    """
    Scan corpus (HTML + all JS) with regex patterns.
    Returns {label: [unique_matches]} — deduped, top 10 per label.
    """
    out = {}
    for label, pat in patterns.items():
        raw = re.findall(pat, corpus, re.IGNORECASE | re.DOTALL)
        # Flatten tuples (from groups)
        flat = []
        for m in raw:
            if isinstance(m, tuple):
                flat.extend([x for x in m if x and len(x) > 3])
            else:
                if m and len(m) > 3:
                    flat.append(m)
        unique = list(dict.fromkeys(flat))   # preserve order, dedup
        unique = [v.strip().strip('"\'`) ') for v in unique]
        unique = [v for v in unique if v and not v.startswith("//")]
        if unique:
            out[label] = unique[:10]
    return out


def _build_report(title: str, icon: str, domain: str,
                  results: dict, url: str,
                  js_count: int = 0, extra_lines: list = None) -> str:
    """Build a clean Telegram report with results."""
    hdr = (
        f"{icon} *{title}*\n"
        f"🌐 `{domain}`\n"
        f"📄 HTML + `{js_count}` JS files scanned\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
    )
    if not results and not extra_lines:
        return (
            hdr +
            "\n✅ *Nothing found* — Keys may be:\n"
            "  • Loaded via server-side rendering\n"
            "  • Injected at runtime (obfuscated)\n"
            "  • Stored in env vars / CDN config\n\n"
            f"💡 Try `/extract {url}` for deeper scan"
        )
    lines = [hdr]
    if extra_lines:
    ...
    for label, vals in results.items():
        ...
        lines.append("")
    lines.append("⚠️ _For authorized/educational use only_")
    return "\n".join(lines)


async def _run_key_scan(update, context, title: str, icon: str,
                        patterns: dict, max_js: int = 12,
                        post_process=None):
    """Generic runner: fetch → scan → report. Used by all /key commands."""
    if not context.args:
        return None   # caller handles help text
    ...
    if not ok:
        await update.effective_message.reply_text(
            f"🚫 `{reason}`", parse_mode="Markdown"); return False

    uid = update.effective_user.id
    ok2, wait = check_rate_limit(uid)
    if not ok2:
        await update.effective_message.reply_text(
            f"⏳ `{wait}s` စောင့်ပါ", parse_mode="Markdown"); return False

    domain = urlparse(url).hostname
    msg = await update.effective_message.reply_text(
        f"{icon} *{title}*\n🌐 `{domain}`\n\n"
        f"⏳ HTML + JS bundles fetching...",
        parse_mode="Markdown"
    )

    data = await asyncio.to_thread(_scrape_full, url, max_js)

    if not data["html"]:
        await msg.edit_text(
            f"❌ *Fetch failed*\n🌐 `{domain}`\n\n"
            f"Status: `{data['status']}` — URL စစ်ပါ သို့မဟုတ် site inaccessible",
            parse_mode="Markdown"); return False

    corpus    = data["all_text"]
    js_count  = len(data["js_sources"])
    results   = _scan_text(corpus, patterns)
    extra     = post_process(data, results) if post_process else None

    report = _build_report(title, icon, domain, results, url, js_count, extra)
...
# ══════════════════════════════════════════════════
# /sitekey — reCAPTCHA / hCaptcha / Turnstile
# ══════════════════════════════════════════════════
@user_guard
async def cmd_sitekey(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    patterns = {
        "reCAPTCHA v2 (data-sitekey)":
            r'data-sitekey=["\']([A-Za-z0-9_\-]{20,60})["\']',
        "reCAPTCHA v3 (render=)":
            r'render=([6][A-Za-z0-9_\-]{38})',
        "reCAPTCHA v3 (grecaptcha.execute)":
            r'grecaptcha\.execute\s*\(\s*["\']([A-Za-z0-9_\-]{20,60})["\']',
        "reCAPTCHA (inline config)":
            r'["\']sitekey["\']\s*:\s*["\']([6L][A-Za-z0-9_\-]{37,39})["\']',
        "hCaptcha sitekey":
            r'(?:data-sitekey=["\']|hcaptcha[^"\']*sitekey["\']?\s*:\s*["\'])([a-f0-9\-]{36})["\']',
        "Turnstile sitekey":
            r'(?:data-sitekey=["\']|turnstile[^"\']*sitekey["\']?\s*:\s*["\'])([0-9A-Za-z_]{20,60})["\']',
        "Turnstile (0x prefix)":
            r'(0x[A-Z0-9]{20,40})',
        "FunCaptcha / Arkose public key":
            r'(?:public_key|publicKey)\s*:\s*["\']([A-Z0-9]{8}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{12})["\']',
        "Friendly Captcha sitekey":
            r'data-sitekey=["\']([A-Z0-9]{20,30})["\']',
    }

    def _post(data, results):
        notes = []
        if "hcaptcha.com" in data["html"]:
            notes.append("✅ hCaptcha script detected")
        if "recaptcha" in data["html"].lower():
            notes.append("✅ reCAPTCHA script detected")
        if "challenges.cloudflare.com" in data["html"]:
            notes.append("✅ Cloudflare Turnstile script detected")
        return notes or None

    await _run_key_scan(update, context,
                        "Captcha Site Keys", "🔑", patterns,
                        post_process=_post)


# ══════════════════════════════════════════════════
# /apikeys — Generic API Keys & Tokens
# ══════════════════════════════════════════════════
@user_guard
async def cmd_apikeys(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.effective_message.reply_text(
            ...
            "Deep scan (HTML + JS bundles):\n"
            "AWS · Google · OpenAI · GitHub · Slack\n"
            "Mapbox · SendGrid · Twilio · Stripe pub\n"
    ...
    patterns = {
        ...
        "AWS Secret Access Key":
            r'(?:aws_secret|AWS_SECRET_ACCESS_KEY|secret_access_key)\s*[=:]\s*["\']?([A-Za-z0-9+/]{40})["\']?',
        "Google API Key":
            r'(AIza[0-9A-Za-z\-_]{35})',
        "Google OAuth Client ID":
            r'([0-9]{8,20}-[a-z0-9]{32}\.apps\.googleusercontent\.com)',
        "OpenAI Key":
            r'(sk-[A-Za-z0-9]{20,60})',
        "OpenAI Org":
            r'(org-[A-Za-z0-9]{20,40})',
        "GitHub PAT":
            r'(gh[pousr]_[A-Za-z0-9]{36,})',
        "Slack Bot Token":
            r'(xoxb-[0-9]{10,13}-[0-9]{10,13}-[A-Za-z0-9]{24})',
        "Slack User Token":
            r'(xoxp-[0-9A-Za-z\-]{40,80})',
        "Slack Webhook":
            r'(https://hooks\.slack\.com/services/T[A-Z0-9]+/B[A-Z0-9]+/[A-Za-z0-9]+)',
        "Mapbox Token":
            r'(pk\.eyJ[A-Za-z0-9._\-]{30,})',
        "SendGrid API Key":
            r'(SG\.[A-Za-z0-9_\-]{22}\.[A-Za-z0-9_\-]{43})',
        "Twilio Account SID":
            r'(AC[a-f0-9]{32})',
        "Twilio Auth Token":
            r'(?:twilio[^"\']{0,30}["\'])([a-f0-9]{32})(["\'])',
        "Stripe Publishable Key":
            r'(pk_(?:live|test)_[A-Za-z0-9]{24,})',
        "Stripe Secret Key ⚠️":
            r'(sk_(?:live|test)_[A-Za-z0-9]{24,})',
        "Generic Bearer":
            r'[Bb]earer\s+([A-Za-z0-9\-_.]{20,200})',
        "Generic API Key (env)":
            r'(?:api[_\-]?key|API_KEY)\s*[=:]\s*["\']([A-Za-z0-9_\-]{20,80})["\']',
        "Anthropic API Key":
            r'(sk-ant-[A-Za-z0-9\-_]{40,})',
    }

    await _run_key_scan(update, context,
                        "API Keys & Tokens", "🗝", patterns, max_js=15)


# ══════════════════════════════════════════════════
# /firebase — Firebase Config Deep Scan
# ══════════════════════════════════════════════════
@user_guard
async def cmd_firebase(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.effective_message.reply_text(
            ...
            "Extracts full Firebase config block:\n"
            "apiKey · authDomain · projectId\n"
            "storageBucket · messagingSenderId · appId\n"
            "measurementId · databaseURL",
            parse_mode="Markdown"); return

    url = context.args[0] if context.args[0].startswith("http") else "https://" + context.args[0]
    ok, reason = is_safe_url(url)
    if not ok:
    ...
    domain = urlparse(url).hostname
    msg = await update.effective_message.reply_text(
        f"🔥 *Firebase Config Scan*\n🌐 `{domain}`\n\n⏳ Fetching...",
        parse_mode="Markdown"
    )

    data = await asyncio.to_thread(_scrape_full, url, 15)
    if not data["html"]:
        await msg.edit_text("❌ Fetch failed", parse_mode="Markdown"); return

    corpus    = data["all_text"]
    js_count  = len(data["js_sources"])
    lines     = [f"🔥 *Firebase Config*", f"🌐 `{domain}`",
                 f"📄 HTML + `{js_count}` JS scanned", "━━━━━━━━━━━━━━━━━━━━", ""]

    found_any = False

    # ── Method 1: Full config object ─────────────────────────
    config_pat = re.compile(
        r'(?:firebaseConfig|initializeApp|firebase\.initializeApp)\s*[\(=,]\s*\{([^}]{20,800})\}',
        re.DOTALL | re.IGNORECASE
    )
    for m in config_pat.finditer(corpus):
        block = m.group(1)
        cfg = {}
        for field in ["apiKey","authDomain","projectId","storageBucket",
                      "messagingSenderId","appId","measurementId","databaseURL"]:
            fm = re.search(
                rf'["\']?{field}["\']?\s*:\s*["\']([^"\']+)["\']', block
            )
            if fm:
                cfg[field] = fm.group(1)
        if cfg:
            lines.append("✅ *Firebase Config Block Found:*\n")
    ...
    # ── Method 2: Individual pattern fallback ────────────────
    if not found_any:
        individual = {
            "API Key":            r'(AIza[0-9A-Za-z\-_]{35})',
            "Project ID":         r'["\']projectId["\']\s*:\s*["\']([a-z0-9\-]{4,30})["\']',
            "App ID":             r'["\']appId["\']\s*:\s*["\']([0-9:a-z\-]{10,60})["\']',
            "Auth Domain":        r'["\']authDomain["\']\s*:\s*["\']([^"\']{10,60}\.firebaseapp\.com)["\']',
            "Storage Bucket":     r'["\']storageBucket["\']\s*:\s*["\']([^"\']{5,60}\.appspot\.com)["\']',
            "Database URL":       r'(https://[a-z0-9\-]+\.firebaseio\.com)',
            "Messaging Sender":   r'["\']messagingSenderId["\']\s*:\s*["\'](\d{8,15})["\']',
            "Measurement ID":     r'(G-[A-Z0-9]{8,12})',
            "FCM Vapid Key":      r'["\']vapidKey["\']\s*:\s*["\']([A-Za-z0-9_\-]{86,90})["\']',
        }
        results = _scan_text(corpus, individual)
        if results:
            lines.append("⚠️ *Partial config found (no block):*\n")
            for label, vals in results.items():
    ...
    if not found_any:
        lines += [
            "✅ Firebase config not found in source",
            "",
            "_May be loaded server-side or obfuscated_",
            f"\n💡 `/apikeys {url}` — Try generic key scan",
        ]

    lines.append("⚠️ _Educational/authorized use only_")
    report = "\n".join(lines)
    try:
        await msg.edit_text(report, parse_mode="Markdown")
    except Exception:
...
async def cmd_paykeys(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.effective_message.reply_text(
            ...
            "Razorpay · Adyen · 2Checkout\n"
            "KBZPay · AYA Pay · MPU (Myanmar local)",
            parse_mode="Markdown"); return

    patterns = {
        "Stripe Publishable Key":
            r'(pk_(?:live|test)_[A-Za-z0-9]{24,110})',
        "Stripe Secret Key ⚠️":
            r'(sk_(?:live|test)_[A-Za-z0-9]{24,110})',
        "Stripe Webhook Secret":
            r'(whsec_[A-Za-z0-9]{32,64})',
        "PayPal Client ID":
            r'(?:client.id|paypal)[^"\']{0,40}["\']([A-Za-z0-9\-_]{50,100})["\']',
        "PayPal Secret":
            r'(?:client.secret|paypal[_\-]?secret)[^"\']{0,20}["\']([A-Za-z0-9\-_]{50,100})["\']',
        "Braintree Tokenization Key":
            r'(sandbox|production)_[a-z0-9]{8}_[a-z0-9]{16}',
        "Braintree Merchant ID":
            r'(?:merchant.?id|braintree)[^"\']{0,30}["\']([a-z0-9]{16})["\']',
        "Square App ID":
            r'(sq0idp-[A-Za-z0-9_\-]{22})',
        "Square Access Token":
            r'(sq0atp-[A-Za-z0-9_\-]{22})',
        "Razorpay Key ID":
            r'(rzp_(?:live|test)_[A-Za-z0-9]{14,20})',
        "Razorpay Secret ⚠️":
            r'(?:razorpay[_\-]?secret|key_secret)[^"\']{0,20}["\']([A-Za-z0-9]{20,40})["\']',
        "Adyen API Key":
            r'(AQE[a-zA-Z0-9+/]{40,})',
        "2Checkout Account":
            r'(?:2checkout|twocheckout)[^"\']{0,30}["\'](\d{7,10})["\']',
        "KBZPay Merchant Code":
            r'(?:kbz|kpay)[_\-]?(?:merchant|code)[^"\']{0,20}["\']([A-Za-z0-9_\-]{8,30})["\']',
    }

    await _run_key_scan(update, context,
                        "Payment Gateway Keys", "💳", patterns, max_js=12)


# ══════════════════════════════════════════════════
# /socialkeys — Social Media App IDs
# ══════════════════════════════════════════════════
@user_guard
async def cmd_socialkeys(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    patterns = {
        "Facebook App ID":
            r'(?:appId|fb:app_id|facebook[_\-]?app[_\-]?id)[^"\']{0,20}["\']?(\d{10,17})["\']?',
        "Facebook Pixel ID":
            r'fbq\s*\(\s*["\']init["\']\s*,\s*["\']?(\d{10,17})["\']?',
        "Facebook Access Token ⚠️":
            r'(EAAa[A-Za-z0-9]{80,})',
        "Google Client ID":
            r'([0-9]{8,20}-[a-z0-9]{32}\.apps\.googleusercontent\.com)',
        "Google Sign-In":
            r'data-client_id=["\']([0-9]{8,20}-[a-z0-9]{32}\.apps\.googleusercontent\.com)["\']',
        "Twitter/X Consumer Key":
            r'(?:consumer_key|twitter[_\-]?key)[^"\']{0,20}["\']([A-Za-z0-9]{25,35})["\']',
        "Twitter/X Bearer":
            r'(?:twitter[^"\']*bearer|TWITTER_BEARER)[^"\']{0,20}["\']([A-Za-z0-9%]{50,})["\']',
        "LinkedIn Partner ID":
            r'(?:_linkedin_partner_id|linkedin[_\-]?id)[^"\']{0,20}["\']?(\d{6,10})["\']?',
        "LinkedIn Client ID":
            r'(?:linkedin[_\-]?client[_\-]?id)[^"\']{0,20}["\']([A-Za-z0-9]{10,20})["\']',
        "TikTok Pixel":
            r'(?:ttq\.load|tiktok[_\-]?pixel)[^"\']{0,30}["\']([A-Z0-9]{15,20})["\']',
        "Pinterest Tag ID":
            r'pintrk\s*\(\s*["\']load["\']\s*,\s*["\']?(\d{10,15})["\']?',
        "Snapchat Pixel":
            r'snaptr\s*\(\s*["\']init["\']\s*,\s*["\']([a-f0-9\-]{36})["\']',
        "Discord Client ID":
            r'(?:discord[_\-]?client[_\-]?id|discord\.com/api/oauth2)[^"\']{0,30}["\']?(\d{17,19})["\']?',
    }

    await _run_key_scan(update, context,
                        "Social Media App IDs", "📱", patterns, max_js=12)
...
async def cmd_analytics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.effective_message.reply_text(
            ...
            "GA4 · UA · GTM · Google Ads · Bing\n"
            "Hotjar · Mixpanel · Amplitude · Heap\n"
            "Segment · Clarity · Intercom · HubSpot",
            parse_mode="Markdown"); return
    ...
    patterns = {
        ...
        "Google Optimize":
            r'(OPT-[A-Z0-9]{7})',
        "Bing UET Tag":
            r'(?:uetq\.push|bing[_\-]?tag)[^"\']{0,30}["\']?(\d{7,10})["\']?',
        "Hotjar Site ID":
            r'(?:hjid|hotjar)[^"\']{0,30}["\']?(\d{5,12})["\']?',
        "Mixpanel Token":
            r'(?:mixpanel\.init|mixpanel_token)\s*\(\s*["\']([a-f0-9]{32})["\']',
        "Amplitude API Key":
            r'(?:amplitude|Amplitude)\.init\s*\(\s*["\']([a-f0-9]{32})["\']',
        "Heap Analytics ID":
            r'heap\.load\s*\(\s*["\'](\d{8,12})["\']',
        "Segment Write Key":
            r'analytics\.load\s*\(\s*["\']([A-Za-z0-9]{20,40})["\']',
        "Microsoft Clarity":
            r'clarity\s*\(\s*["\']set["\']\s*,\s*["\']([a-z0-9]{10,15})["\']',
        "HubSpot Portal ID":
            r'(?:hubspot|_hsp)[^"\']{0,50}portalId[^"\']{0,20}["\']?(\d{6,10})["\']?',
        "FullStory Org ID":
            r'FS\.identify|_fs_org\s*=\s*["\']([A-Z0-9]{6,10})["\']',
        "Crisp Website ID":
            r'CRISP_WEBSITE_ID\s*=\s*["\']([a-f0-9\-]{36})["\']',
    }

    await _run_key_scan(update, context,
                        "Analytics & Tracking IDs", "📊", patterns, max_js=10)


# ══════════════════════════════════════════════════
# /hiddenkeys — Hardcoded Secrets / Credentials
# ══════════════════════════════════════════════════
@user_guard
async def cmd_hiddenkeys(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.effective_message.reply_text(
            ...
            "Passwords · Private Keys · DB URIs\n"
            "JWT secrets · .env leaks · SSH keys",
            parse_mode="Markdown"); return

    patterns = {
        "Hardcoded Password":
            r'(?:password|passwd|pwd|pass)\s*[=:]\s*["\']([^"\']{6,60})["\']',
        "DB Connection String":
            r'(?:mongodb(?:\+srv)?|mysql|postgres(?:ql)?|redis|mssql)://[A-Za-z0-9_%@.:/?=&\-]{10,200}',
        "Private Key (PEM header)":
            r'(-----BEGIN (?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----)',
        "JWT Secret":
            r'(?:JWT_SECRET|jwt[_\-]?secret|jwtSecret)\s*[=:]\s*["\']([^"\']{8,80})["\']',
        "Secret Key / Token":
            r'(?:SECRET_KEY|secretKey|secret_key|APP_SECRET)\s*[=:]\s*["\']([^"\']{8,80})["\']',
        "Encryption Key":
            r'(?:encrypt(?:ion)?[_\-]?key|ENCRYPT_KEY)\s*[=:]\s*["\']([^"\']{8,80})["\']',
        "AWS Secret Access Key ⚠️":
            r'(?:aws_secret_access_key|AWS_SECRET)[^"\']{0,10}["\']([A-Za-z0-9+/]{40})["\']',
        "Private Key (hex 32B)":
            r'(?:private[_\-]?key|PRIVATE_KEY)\s*[=:]\s*["\']([a-fA-F0-9]{64})["\']',
        "SSH Private Key":
            r'(ssh-rsa AAAA[A-Za-z0-9+/]{30,})',
        "Basic Auth Credentials":
            r'(?:Authorization\s*:\s*Basic\s+)([A-Za-z0-9+/=]{20,})',
        "Hardcoded Token":
            r'(?:access_token|ACCESS_TOKEN|auth_token)\s*[=:]\s*["\']([A-Za-z0-9_\-\.]{20,200})["\']',
        ".env Variable Leak":
            r'(?:process\.env\.|os\.environ\.get\s*\()([A-Z_]{4,40})',
    }

    await _run_key_scan(update, context,
                        "Hidden Secrets & Credentials", "🔍", patterns, max_js=15)
...
async def cmd_endpoints(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.effective_message.reply_text(
            ...
            "Deep extract from HTML + JS bundles:\n"
            "REST API paths · GraphQL · WebSocket\n"
            "Internal paths · Auth routes · Admin routes",
            parse_mode="Markdown"); return
    ...
    # Categorized endpoint extraction
    api_paths   = set()
    auth_paths  = set()
    admin_paths = set()
    ws_urls     = set()
    ...
    # WebSocket
    for m in re.finditer(r'["\`](wss?://[^\s"\'`<>]{8,150})["\`]', corpus):
        ws_urls.add(m.group(1))

    # GraphQL
    for m in re.finditer(r'["\`]((?:https?://)?[^\s"\'`<>]*/graphql[^\s"\'`<>]{0,50})["\`]', corpus, re.I):
        graphql_eps.add(m.group(1).rstrip(".,;"))

    # Auth/login paths
    for m in re.finditer(
        r'["\`](/(?:auth|login|logout|signin|signup|register|oauth|token|refresh|verify'
        r'|password|forgot|reset|confirm|activate|sso|saml|oidc)[^\s"\'`<>]{0,80})["\`]',
        corpus, re.I
    ):
        p = m.group(1).rstrip(".,;")
        if len(p) > 2:
    ...
    # Admin paths
    for m in re.finditer(
        r'["\`](/(?:admin|dashboard|manage|cms|control|backoffice|panel|staff|internal'
        r'|superuser|moderator)[^\s"\'`<>]{0,80})["\`]',
        corpus, re.I
    ):
        p = m.group(1).rstrip(".,;")
        if len(p) > 2:
    ...
    # API paths
    for m in re.finditer(
        r'["\`](/(?:api|rest|v\d+|service|endpoint|rpc|gql'
        r'|user|account|product|order|cart|search|upload|download|webhook|callback'
        r'|public|private|data|feed|export|import|batch|bulk)[^\s"\'`<>]{0,100})["\`]',
        corpus, re.I
    ):
        p = m.group(1).rstrip(".,;")
        if len(p) > 3 and p not in auth_paths and p not in admin_paths:
            api_paths.add(p)

    # Full API URLs on same domain
    for m in re.finditer(r'["\`](https?://[^\s"\'`<>]{8,200})["\`]', corpus):
        u = m.group(1)
        try:
            pu = urlparse(u)
            if pu.netloc == domain and any(
                k in pu.path for k in ["/api/", "/v1/", "/v2/", "/rest/", "/graphql"]
            ):
                full_api.add(pu.path + ("?" + pu.query if pu.query else ""))
        except Exception:
            pass
    ...
    total = (len(api_paths) + len(auth_paths) + len(admin_paths)
             + len(ws_urls) + len(graphql_eps) + len(full_api))

    if total == 0:
        lines += [
            "✅ No hardcoded endpoints found",
            "",
            "_Paths may be runtime-generated or obfuscated_",
            f"\n💡 `/api {url}` — Active probing discovery",
        ]
    else:
        lines.append(f"📊 *Total unique: `{total}`*\n")
        if graphql_eps:
            lines.append(f"⚡ *GraphQL ({len(graphql_eps)}):*")
            for e in sorted(graphql_eps)[:5]:
                lines.append(f"  `{e[:80]}`")
            lines.append("")
        if ws_urls:
            lines.append(f"🔌 *WebSocket ({len(ws_urls)}):*")
            for e in sorted(ws_urls)[:5]:
                lines.append(f"  `{e[:80]}`")
            lines.append("")
        if admin_paths:
            lines.append(f"👑 *Admin/Dashboard ({min(len(admin_paths),10)}):*")
            for p in sorted(admin_paths)[:10]:
                lines.append(f"  `{p[:80]}`")
            lines.append("")
        if auth_paths:
            lines.append(f"🔐 *Auth Routes ({min(len(auth_paths),10)}):*")
            for p in sorted(auth_paths)[:10]:
                lines.append(f"  `{p[:80]}`")
            lines.append("")
        if api_paths or full_api:
            all_api = sorted((api_paths | full_api))[:25]
            lines.append(f"🛤 *API Paths ({len(all_api)}):*")
            for p in all_api:
                lines.append(f"  `{p[:80]}`")
            lines.append("")
...
async def cmd_jwtlive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.effective_message.reply_text(
            "🎟 *Usage:* `/jwtlive https://example.com`\n\n"
            "Hunts for exposed JWT tokens:\n"
            "• Response headers (Authorization, Set-Cookie)\n"
            "• HTML source & inline JS\n"
            "• Linked JS bundle files\n"
            "• Decodes payload (sub, email, role, exp)",
            parse_mode="Markdown"); return
    ...
    data = await asyncio.to_thread(_scrape_full, url, 8)
    ...
    def _decode_jwt_safe(token: str) -> dict:
        try:
            ...
            payload = json.loads(_b64.urlsafe_b64decode(pad).decode("utf-8", errors="replace"))
            return payload if isinstance(payload, dict) else {}
        except Exception:
            return {}

    def _fmt_payload(pl: dict) -> list:
        rows = []
        interest = ["sub","email","role","roles","scope","aud","iss","exp","iat","uid","user_id","name"]
        for k in interest:
            if k in pl:
                v = str(pl[k])
                if k == "exp":
                    try:
                        v = datetime.utcfromtimestamp(int(v)).strftime("%Y-%m-%d %H:%M UTC")
                    except Exception:
                        pass
                rows.append(f"    ↳ `{k}`: `{v[:60]}`")
        return rows

    lines = [
        f"🎟 *JWT Token Hunter*",
        f"🌐 `{domain}`",
        f"📄 HTML + `{len(data['js_sources'])}` JS scanned",
        "━━━━━━━━━━━━━━━━━━━━",
        ""
    ]
    found_count = 0
    ...
    header_jwts = []
    for hname, hval in data["headers"].items():
        for m in JWT_PAT.finditer(hval):
            header_jwts.append((hname, m.group(1)))

    # ── 2. Check Set-Cookie ──────────────────────────────────
    cookie_jwts = []
    for cname, cval in data["cookies"].items():
        for m in JWT_PAT.finditer(cval):
            cookie_jwts.append((cname, m.group(1)))

    # ── 3. Scan HTML + JS ────────────────────────────────────
    source_jwts = []
    seen_tokens = set()
    for m in JWT_PAT.finditer(data["all_text"]):
        t = m.group(1)
        if t not in seen_tokens:
            seen_tokens.add(t)
            source_jwts.append(t)

    if header_jwts:
        lines.append("🔴 *Found in Response Headers:*")
        for hname, token in header_jwts[:3]:
            lines.append(f"  Header: `{hname}`")
            lines.append(f"  Token: `{token[:50]}...`")
            pl = _decode_jwt_safe(token)
            lines.extend(_fmt_payload(pl))
            lines.append("")
        found_count += len(header_jwts)

    if cookie_jwts:
        lines.append("🍪 *Found in Cookies:*")
        for cname, token in cookie_jwts[:3]:
            lines.append(f"  Cookie: `{cname}`")
            lines.append(f"  Token: `{token[:50]}...`")
            pl = _decode_jwt_safe(token)
            lines.extend(_fmt_payload(pl))
            lines.append("")
        found_count += len(cookie_jwts)

    if source_jwts:
        lines.append(f"📄 *Found in Source/JS ({len(source_jwts)}):*")
        for token in source_jwts[:5]:
            lines.append(f"  `{token[:55]}...`")
            pl = _decode_jwt_safe(token)
            if pl:
                lines.extend(_fmt_payload(pl))
            lines.append("")
        found_count += len(source_jwts)

    if found_count == 0:
        lines += [
            "✅ No exposed JWT tokens found",
            "",
            "_Tokens are likely only sent after authentication_",
            "",
            f"💡 `/jwtattack <token>` — If you have a token, attack it",
        ]
    else:
        lines.append(f"📊 Total tokens found: `{found_count}`")
        lines.append(f"\n💡 `/jwtattack <token>` — Full JWT attack on a token")
...
# ══════════════════════════════════════════════════
# /pushkeys — Push Notification Keys
# ══════════════════════════════════════════════════
@user_guard
async def cmd_pushkeys(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.effective_message.reply_text(
            ...
            "FCM · OneSignal · Pusher · Ably\n"
            "VAPID WebPush · Beams (Pusher)\n"
            "Includes JS bundle scan",
            parse_mode="Markdown"); return

    patterns = {
        "FCM / Firebase Server Key ⚠️":
            r'["\']([A-Za-z0-9\-_:]{100,200})["\']',   # very long token pattern
        "FCM Sender ID":
            r'(?:messagingSenderId|gcm_sender_id|sender[_\-]?id)\s*[=:]\s*["\']?(\d{10,15})["\']?',
        "FCM vapidKey":
            r'vapidKey\s*:\s*["\']([A-Za-z0-9_\-]{86,90})["\']',
        "VAPID Public Key":
            r'(?:applicationServerKey|vapid[_\-]?public|pushPublicKey)\s*[=:]\s*["\']([A-Za-z0-9_\-]{86,90})["\']',
        "OneSignal App ID":
            r'(?:OneSignal\.init|appId\s*:\s*)["\']([a-f0-9\-]{36})["\']',
        "OneSignal Safari Web ID":
            r'safari_web_id\s*:\s*["\']([A-Za-z0-9._\-]{20,80})["\']',
        "Pusher App Key":
            r'new\s+Pusher\s*\(\s*["\']([a-z0-9]{8,20})["\']',
        "Pusher Cluster":
            r'cluster\s*:\s*["\']([a-z]{2,5}[0-9]?)["\']',
        "Pusher Beams Instance":
            r'(?:PusherBeams|beams)[^"\']{0,40}instanceId[^"\']{0,20}["\']([a-f0-9\-]{36})["\']',
        "Ably API Key":
            r'([A-Za-z0-9]{6,12}\.[A-Za-z0-9]{6,12}:[A-Za-z0-9_\-]{32,})',
        "Web Push SW Registration":
            r'serviceWorker\.register\s*\(\s*["\']([^"\']+)["\']',
    }

    await _run_key_scan(update, context,
                        "Push Notification Keys", "🔔", patterns, max_js=12)


# ══════════════════════════════════════════════════
# /chatkeys — Live Chat Widget Keys
# ══════════════════════════════════════════════════
@user_guard
async def cmd_chatkeys(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.effective_message.reply_text(
            ...
            "LiveChat · Freshdesk · Drift · HubSpot\n"
            "Also scans JS bundles",
            parse_mode="Markdown"); return

    patterns = {
        "Intercom App ID":
            r'(?:app_id|intercom)[^"\']{0,40}["\']([a-z0-9]{8,15})["\']',
        "Intercom API Key ⚠️":
            r'(?:intercom[_\-]?api[_\-]?key|intercomToken)\s*[=:]\s*["\']([A-Za-z0-9_\-]{20,80})["\']',
        "Crisp Website ID":
            r'(?:CRISP_WEBSITE_ID|crisp[_\-]?id)\s*[=:]*\s*["\']([a-f0-9\-]{36})["\']',
        "Zendesk Subdomain":
            r'(?:zopim|zendesk|zdChat)[^"\']{0,60}["\']([a-zA-Z0-9\-]+\.zendesk\.com|[a-zA-Z0-9\-]+\.zopim\.com)["\']',
        "Zendesk Key":
            r'zE\s*\(\s*["\']webWidget["\']\s*,\s*["\']identify["\']\s*,\s*\{([^}]+)\}',
        "Tawk.to Property":
            r'Tawk_API[^;]{0,100}["\']([a-f0-9]{24}/[a-z0-9\-]{36})["\']',
        "LiveChat License":
            r'(?:__lc\.license|livechat[_\-]?license)\s*=\s*["\']?(\d{6,10})["\']?',
        "Freshchat / Freshdesk Token":
            r'(?:freshchat|freshdesk)[^"\']{0,40}["\']([A-Za-z0-9\-_]{20,60})["\']',
        "Drift API / App":
            r'driftt\.com/include/["\']?([A-Za-z0-9]{10,30})["\']?',
        "HubSpot Portal ID":
            r'hs-script-loader[^"\']{0,60}["\']?(\d{6,10})["\']?',
        "Olark Site ID":
            r'(?:olark\.identify|olark)[^"\']{0,30}["\']([a-z0-9\-]{12,20})["\']',
        "Tidio Key":
            r'tidio[^"\']{0,30}["\']([a-z0-9]{32})["\']',
    }

    await _run_key_scan(update, context,
                        "Live Chat & Support Keys", "💬", patterns, max_js=10)


# ══════════════════════════════════════════════════
# /proxy — Proxy Pool Management (Admin)
# ══════════════════════════════════════════════════
@admin_only
async def cmd_proxy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/proxy [reload|list|add <url>|test <url>]"""
    args = context.args or []
    ...
    if sub == "reload":
        msg = await update.effective_message.reply_text("🔄 Reloading proxy list...")
        await asyncio.to_thread(proxy_manager.reload)
        st2 = proxy_manager.stats()
        await msg.edit_text(
            f"✅ *Proxy Pool Reloaded*\n\n"
            f"Total: `{st2['total']}` | Live: `{st2['live']}`\n"
            f"Available: `{st2['available']}` | Cooldown: `{st2['in_cooldown']}`\n"
            f"Source: `{str(st2['source'])[:60]}`",
            parse_mode="Markdown"
        ); return
        ...
        if not proxies:
            await update.effective_message.reply_text(
                "📭 *No proxies loaded*\n\n"
                "Use `/setproxy <url>` to set a proxy list source\n"
                "or add proxies to `proxies.txt`",
                parse_mode="Markdown"); return
        lines = [f"📋 *Proxy List ({len(proxies)} total)*\n"]
        for i, p in enumerate(proxies[:25], 1):
            if p["alive"] and not p["cooldown"]:
                icon = "✅"
            elif p["cooldown"]:
                icon = "⏳"
            else:
                icon = "❌"
            lines.append(f"{icon} `{i:02d}` `{p['proxy'][:55]}`")
        if len(proxies) > 25:
            lines.append(f"\n_...and {len(proxies)-25} more_")
        await update.effective_message.reply_text(
        ...
        await update.effective_message.reply_text(
            f"➕ *Proxy added*\n`{proxy_url}`\n\n"
            f"Background health check running...",
            parse_mode="Markdown"); return
        ...
        if "://" not in proxy_url:
            proxy_url = "http://" + proxy_url
        msg = await update.effective_message.reply_text(
            f"🧪 Testing `{proxy_url}`...", parse_mode="Markdown")
        def _test():
            try:
        ...
        alive, detail = await asyncio.to_thread(_test)
        icon = "✅" if alive else "❌"
        await msg.edit_text(
            f"{icon} *Proxy Test Result*\n\n"
            f"Proxy: `{proxy_url}`\n"
            f"Status: `{'ALIVE' if alive else 'DEAD'}`\n"
            f"Info: `{detail}`",
            parse_mode="Markdown"); return

    # Default: status panel
    enabled_str = "✅ Enabled" if st["enabled"] else "❌ Disabled (PROXY_ENABLED=false)"
    bar_live  = "█" * min(st["live"], 20) + "░" * max(0, 20 - min(st["live"], 20))
    pct       = f"{st['live']}/{st['total']} ({int(st['live']/max(st['total'],1)*100)}%)" if st["total"] else "0/0"
    ...
    await update.effective_message.reply_text(
        ...
        f"Live:  `{pct}`\n"
        f"`{bar_live}`\n"
        f"Available: `{st['available']}` | Cooldown: `{st['in_cooldown']}`\n"
        f"Last check: `{st['last_load']}`\n"
        f"Source: `{str(st['source'])[:60]}`\n\n"
        f"*Commands:*\n"
        f"  `/proxy reload` — Reload + health check all\n"
        f"  `/proxy list` — List all proxies\n"
        f"  `/proxy add http://ip:port` — Add proxy\n"
        f"  `/proxy test http://ip:port` — Test specific proxy\n"
        f"  `/setproxy <url>` — Set remote list URL",
        parse_mode="Markdown"
    )


# ══════════════════════════════════════════════════
# /setproxy — Set Remote Proxy List URL (Admin)
# ══════════════════════════════════════════════════
@admin_only
async def cmd_setproxy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.effective_message.reply_text(
            f"🔗 *Proxy Source Config*\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"Current source:\n`{PROXY_FILE_URL or 'Local file: ' + PROXY_FILE}`\n\n"
            f"*Usage:* `/setproxy <raw_url>`\n\n"
            f"Example (GitHub raw):\n"
            f"`/setproxy https://raw.githubusercontent.com/user/repo/main/proxies.txt`\n\n"
            f"Format (one per line):\n"
            f"`1.2.3.4:8080`\n"
            f"`http://1.2.3.4:3128`\n"
            f"`socks5://1.2.3.4:1080`",
            parse_mode="Markdown"); return
    ...
    status_icon = "✅" if st["live"] > 0 else "⚠️"
    await msg.edit_text(
        f"{status_icon} *Proxy Source Updated*\n\n"
        f"URL: `{new_url[:60]}`\n\n"
        f"Live: `{st['live']}/{st['total']}` proxies\n"
        f"Available: `{st['available']}`\n\n"
        f"{'✅ Ready to use' if st['live'] > 0 else '⚠️ No live proxies — check URL format'}",
        parse_mode="Markdown"
    )

def _fetch_html_for_keys(url: str) -> tuple:
    """Fetch HTML + headers for key extraction. Returns (html, headers_dict, status)"""
    try:
        resp = _req_with_proxy(
...
@admin_only
async def cmd_resetlimit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/resetlimit <user_id> — Reset user's daily download count to 0"""
    if not context.args:
        await update.effective_message.reply_text(
            "🔄 *Usage:* `/resetlimit <user_id>`\n\n"
            "User ၏ daily count ကို 0 ပြန်သတ်မည်\n"
            "User list: `/allusers`",
            parse_mode='Markdown'
        ); return
    ...
    async with db_lock:
        db = _load_db_sync()
        if target not in db["users"]:
            await update.effective_message.reply_text(f"❌ User `{target}` မတွေ့ပါ", parse_mode='Markdown')
            return
        u = db["users"][target]
        old_count = u.get("count_today", 0)
        u["count_today"] = 0
